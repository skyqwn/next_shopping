...
import { searchAlgoliaClient } from "@/lib/algoliaIndex";
...
const Algolia = () => {
  ...
  return (
    <InstantSearchNext
      ...
      indexName="products"
      searchClient={searchAlgoliaClient}
    >
...
function Hit({
  ...
  return (
    <div className="p-4 mb-2 hover:bg-secondary">
      <Link
        href={`/products/${hit.objectId}?id=${hit.objectId}&prodcutId=${hit.id}&price=${hit.price}&title=${hit.title}&type=${hit.productType}&image=${hit.variantImages[0]}&variantId=${hit.objectId}`}
      >
